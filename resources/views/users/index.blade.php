                ...
                <tbody>
                    @foreach ($users as $user)
                        ...
                        <tr class="border-t border-asphalt-600 {{ $editing?->is($user) ? 'bg-asphalt-700' : '' }}">
                            ...
                            <td class="px-5 py-3 font-mono text-[13px] text-plate-300">{{ $user->email }}</td>
                            <td class="px-5 py-3">
                                <x-status-badge :status="$user->isAdmin()
                                    ? ['level' => 'warn', 'label' => 'Admin']
                                    : ['level' => 'ok', 'label' => 'Clerk']" />
                            </td>
                            <td class="px-5 py-3 tabular-nums text-plate-300">{{ $user->created_at?->format('d M Y') }}</td>
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('users.destroy', $user) }}"
                                              onsubmit="return confirm('Remove access for {{ $user->name }}? Readings they logged are kept.')">
                                            @csrf @method('DELETE')
